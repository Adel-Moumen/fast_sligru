...
class GRU_Layer(torch.nn.Module):
    ...
    def _ligru_cell(self, w, ht):
        ...
        for k in range(w.shape[1]):
            ...
            zt = cos_activation(wzt + uzt)
            rt = sin_activation(wrt + urt)
...
class LiGRU(torch.nn.Module):
    ...
    def __init__(
        ...
        if self.bidirectional:
            self.w = nn.Linear(self.hidden_size * 2, 2 * self.hidden_size, bias=False)
        else:
...
class LiGRU_Layer(torch.nn.Module):
    ...
    def __init__(
        ...
        # we share U across all layers
        self.u = nn.Linear(self.hidden_size, 2 * self.hidden_size, bias=False)
        self.ln = nn.LayerNorm(self.hidden_size * 2, elementwise_affine=False)
